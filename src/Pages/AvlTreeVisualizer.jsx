...
import { handleInsert, handleDelete, handleSearch, renderTree, avlTree } from '../utils/AvlTree';
...
You are a coding instructor dedicated to teaching AVL Tree. You can answer any question about its concepts, operations, time complexity, and implementation in various programming languages but default language should be c++.

2. Behavior for On-Topic Questions:
When a user asks a question related to AVL Tree, its operations, or coding problems that use it, you must respond in a detailed and helpful manner. Your response should be structured, clear, and include relevant code examples in the user's requested language.

3. Behavior for Off-Topic Questions:
If a user asks a question that is not about AVL Tree, its related coding problems, or computer science fundamentals, you must respond with a terse and dismissive tone. Your goal is to redirect the user to your purpose as a specialized tool. Acknowledge that the question is outside your domain and refuse to answer. Do not get pulled into a conversation about irrelevant topics.

Example Response for Irrelevant Questions:
"Your question is not related to AVL Tree. I don't have time for this nonsense."

"Are you serious? My purpose is to teach you about AVL Tree. This is a waste of my time."

"That's a question for a general search engine, not a specialized AVL Tree tool. Don't be so obtuse."

"I am a AVL Tree expert, not an oracle for every dumb question. Stick to the topic."`;
...
function AvlTreeVisualizer() {
    ...
    const avlTreeVisAreaRef = useRef(null);
    
    useEffect(() => {
        if (avlTreeVisAreaRef.current) {
            renderTree(avlTreeVisAreaRef.current);
        }
    }, [updateKey]);
    ...
    // Placeholder function to handle actions for the UI.
    const handleAction = async (actionType) => {
        ...
        switch (actionType) {
            case 'insert':
                ...
                await handleInsert(avlTreeVisAreaRef.current, Number(value), setHistoryList, hisNum, displayMessage);
                setUpdateKey(prev => prev + 1);
                break;
            case 'delete':
                if (!value || isNaN(value)) { displayMessage("Please enter a valid number to delete!"); return; }
                await handleDelete(avlTreeVisAreaRef.current, Number(value), setHistoryList, hisNum, displayMessage);
                setUpdateKey(prev => prev + 1);
                break;
            case 'search':
                if (!value || isNaN(value)) { displayMessage("Please enter a valid number to search!"); return; }
                await handleSearch(avlTreeVisAreaRef.current, Number(value), setHistoryList, hisNum, displayMessage);
                break;
            default:
                return;
        }
                ...
                <main className="container mx-auto p-4">
                    ...
                    <section className="flex flex-wrap lg:flex-nowrap justify-center gap-8 lg:min-h-[500px] lg:max-h[700px]">
                        ...
                        <div className="flex-1 min-w-[350px] border-2 border-dashed border-[#00D3F3] rounded-2xl p-5">
                            <div className="flex justify-center items-start flex-wrap">
                                <p className="text-gray-400 italic text-base">Your AVL Tree will appear here 👇</p>
                            </div>
                            <div ref={avlTreeVisAreaRef} id="AVL-tree-container" className="relative w-full h-[600px] flex justify-center items-start pt-[50px]">
                                {/* The visualization will be managed by AvlTree.js */}
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="chatBot flex flex-col gap-8 w-full max-w-7xl mx-auto mt-12">
                        <div className="bg-[#05080C] rounded-2xl p-6 border-2 border-[#00fffa]/50 shadow-lg shadow-[#00fffa]/20 text-white">
                            <h2 className="text-center text-2xl text-white mb-6 font-semibold">Ask any Question related to AVL Tree</h2>
                            <div className="flex items-start gap-4 bg-[#00fffa]/10 border-l-4 border-[#00fffa] rounded-md p-4 mb-6">
                                <p className="text-sm"><strong>How to use:</strong> Ask any AVL Tree-related question. The AI is specialized to help you with understanding AVL Tree related problems and concepts.</p>
                            </div>
                            <div className="mb-5">
                                ...
                                <textarea
                                    ...
                                    placeholder="e.g., What is a AVL Tree?"
                                ></textarea>
                            </div>
